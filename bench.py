...
def sweep(args, device):
    """One sweep table per requested fan-in."""
    for k in args.fan_in:
        sweep_one(args, device, k)
        print()


def sweep_one(args, device, K):
    ...
    1.0) is the point where every soma reads the entire input. K therefore sets
    where the sweep ends as well as how wide each dendrite is.
    ...
    print(f"  {'D':>4} {'M':>7} {'G':>4} {'cover':>7} {'seen':>6} {'used':>7} "
          f"{'params':>9} {'MACs/vec':>10} {'fwd(ms)':>8} {'fwd+bwd':>9} "
          f"{'peakMB':>8}   vs dense")
...
def main():
    ...
    ap.add_argument("--fan-in", default="16",
                    help="inputs per dendrite (K); comma-separated for several, "
                         "e.g. --fan-in 8,16,32")
    ...
    ks = ",".join(str(k) for k in args.fan_in)
    print(f"torch {torch.__version__} | {device} ({name}) | K={ks}\n")
    ...
    for B, N, M, S in configs:
        ...
        for K in args.fan_in:
            m = DendriticLinear(N, S, fan_in=K, dendrites_per_soma=D).to(device)
            info = m.sparsity()
            label = f"dendritic K={K}"
            if info["inputs_covered"] < N:
                label += f" ({info['inputs_covered']}/{N} covered)"
            measure(m, x, device, label, info["macs"], base)
